...
function CNTLab(props) {
    ...
    useEffect(() => {
        ...
        getAllCNTLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllCNTLabBookings = async () => {
        try {
            const response = await labApi.getAllCNTLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const CNTBooking = async (values) => {
        try {
            const response = await labApi.CNTLabBooking(values);
            if (response.ok) {

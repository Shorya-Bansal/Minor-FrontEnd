...
function GenericCompLab(props) {
    ...
    useEffect(() => {
        ...
        getAllGenericCompLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllGenericCompLabBookings = async () => {
        try {
            const response = await labApi.getAllGenericCompLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const GenericCompBooking = async (values) => {
        try {
            const response = await labApi.GenericCompLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={GenericCompBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

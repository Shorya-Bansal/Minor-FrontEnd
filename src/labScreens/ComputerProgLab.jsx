...
function ComputerProgLab(props) {
    ...
    const getAllCompProgrammingLabBookings = async () => {
        try {
            const response = await labApi.getAllCompProgrammingLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={CompProgrammingBooking}
                            bookings={bookings}
                        />
                    })}
                </div>
...
export default ComputerProgLab;

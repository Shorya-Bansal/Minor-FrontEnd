...
function AppliedChemLab(props) {
    ...
    useEffect(() => {
        ...
        getAllAppliedChemLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllAppliedChemLabBookings = async () => {
        try {
            const response = await labApi.getAllAppliedChemLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const AppliedChemBooking = async (values) => {
        try {
            const response = await labApi.AppliedChemLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={AppliedChemBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

...
function BEEALab(props) {
    ...
    useEffect(() => {
        ...
        getAllBEEALabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllBEEALabBookings = async () => {
        try {
            const response = await labApi.getAllBEEALabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const BEEABooking = async (values) => {
        try {
            const response = await labApi.BEEALabBooking(values);
            if (response.ok) {

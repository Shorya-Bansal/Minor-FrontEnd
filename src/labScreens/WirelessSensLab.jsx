...
function WirelessSenslab(props) {
    ...
    useEffect(() => {
        ...
        getAllWirelessSensLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllWirelessSensLabBookings = async () => {
        try {
            const response = await labApi.getAllWirelessSensLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const WirelessSensBooking = async (values) => {
        try {
            const response = await labApi.WirelessSensLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={WirelessSensBooking}
                            bookings={bookings}
                        />
                    })}
                </div>
...
export default WirelessSenslab;

...
function EBusinessLab(props) {
    ...
    useEffect(() => {
        ...
        getAllEBusinessLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllEBusinessLabBookings = async () => {
        try {
            const response = await labApi.getAllEBusinessLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const EBusinessBooking = async (values) => {
        try {
            const response = await labApi.EBusinessLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={EBusinessBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

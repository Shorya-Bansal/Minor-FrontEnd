...
function AdvanceNetLab(props) {
    ...
    useEffect(() => {
        ...
        getAllAdvanceNetLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllAdvanceNetLabBookings = async () => {
        try {
            const response = await labApi.getAllAdvanceNetLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const AdvanceNetBooking = async (values) => {
        try {
            const response = await labApi.AdvanceNetLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={AdvanceNetBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

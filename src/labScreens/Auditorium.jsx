...
function Auditorium(props) {
    ...
    const getAllAuditoriumBookings = async () => {
        try {
            const response = await labApi.getAllAuditoriumBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const auditoriumBooking = async (values) => {
        try {
            const response = await labApi.auditoriumBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={auditoriumBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

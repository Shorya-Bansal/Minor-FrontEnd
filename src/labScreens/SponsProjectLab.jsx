...
function SponsProjectLab(props) {
    ...
    useEffect(() => {
        ...
        getAllSponsProjectLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllSponsProjectLabBookings = async () => {
        try {
            const response = await labApi.getAllSponsProjectLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const SponsProjectBooking = async (values) => {
        try {
            const response = await labApi.SponsProjectLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={SponsProjectBooking}
                            bookings={bookings}
                        />
                    })}
                </div>
...
export default SponsProjectLab;

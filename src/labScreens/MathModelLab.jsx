...
function MathModelLab(props) {
    ...
    useEffect(() => {
        ...
        getAllMathModelLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllMathModelLabBookings = async () => {
        try {
            const response = await labApi.getAllMathModelLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const MathModelBooking = async (values) => {
        try {
            const response = await labApi.MathModelLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={MathModelBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

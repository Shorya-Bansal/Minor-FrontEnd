...
function DigitalLogicLab(props) {
    ...
    const getAllDigitalLogicLabBookings = async () => {
        try {
            const response = await labApi.getAllDigitalLogicLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const DigitalLogicBooking = async (values) => {
        try {
            const response = await labApi.digitalLogicLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={DigitalLogicBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

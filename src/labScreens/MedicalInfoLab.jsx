...
function MedicalInfoLab(props) {
    ...
    useEffect(() => {
        ...
        getAllMedicalInfoLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllMedicalInfoLabBookings = async () => {
        try {
            const response = await labApi.getAllMedicalInfoLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const MedicalInfoBooking = async (values) => {
        try {
            const response = await labApi.MedicalInfoLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={MedicalInfoBooking}
                            bookings={bookings}
                        />
                    })}
                </div>

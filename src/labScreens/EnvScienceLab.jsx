...
function EnvScienceLab(props) {
    ...
    useEffect(() => {
        ...
        getAllEnvScienceLabBookings();
    }, [selectedDate]);// eslint-disable-line react-hooks/exhaustive-deps

    const getAllEnvScienceLabBookings = async () => {
        try {
            const response = await labApi.getAllEnvScienceLabBooking(`${selectedDate.getDate()}/${selectedDate.getMonth() + 1}/${selectedDate.getFullYear()}`);
            if (response.ok) {
    ...
    const EnvScienceBooking = async (values) => {
        try {
            const response = await labApi.EnvScienceLabBooking(values);
            if (response.ok) {
                ...
                <div className="lab-booking-div">
                    {timeSlot.map((item, index) => {
                        return <BookingCard
                            ...
                            confirmBooking={EnvScienceBooking}
                            bookings={bookings}
                        />
                    })}
                </div>
